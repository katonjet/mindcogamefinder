...
export default function Genre({params}: {params: {genreid: string;}}) {
    ...
    useEffect(()=>{
        const asyncFn = async () => {
            ...
            try {
                ...
                const data = await (await fetch(`${serverURL}/api/games/bygenre/${genreid}`)).json();
                setGameList(data)

                //setBackColor()
                await DelayLoad(2000);
                if (gameList) {
                    setBackColor(data[0].genre.themecolor)
                    setHeadingTitle(data[0].genre.title)
                }
                setLoading(false)
            } catch (error) {
    ...
    const renderDefault = (<>
        <div className="ml-40 mr-40">
            <H1 className="mt-20 mb-20">{headingTitle} genre</H1>
